...
var binaries = {
  standalone: {
    ...
    filename: 'selenium-server-standalone-' + versions.selenium + '.0.jar',
    url: function() {
      return 'http://selenium-release.storage.googleapis.com/' + versions.selenium + '/' +
          'selenium-server-standalone-' + versions.selenium+ '.0.jar';
    }
  },
  chrome: {
    name: 'chromedriver',
  ...
  ie: {
    ...
    filename: 'IEDriverServer_' + versions.iedriver + '.0.zip',
    url: function() {
      var urlPrefix = 'http://selenium-release.storage.googleapis.com/' + versions.iedriver + '/IEDriverServer';
      if (os.type() == 'Windows_NT') {
        if (os.arch() == 'x64') {
          return urlPrefix + '_x64_' + versions.iedriver + '.0.zip';
        } else {
          return urlPrefix + '_Win32_' + versions.iedriver + '.0.zip';
        }
      }
    }
  }
...
switch (argv._[0]) {
  case 'start':
    ...
    var wdParams = argv._.slice(1);
    if (wdParams.length > 0) {
      args = args.concat(wdParams);
    }
  ...
  case 'update':
    if (argv.standalone) {
      downloadIfNew(binaries.standalone, argv.out_dir, existingFiles, function () {
        if (argv.chrome) {
